...
(function () {
    ...
    let panelVis = { info: true, launch: true, controls: true };

    function readPanelState() {
        const def = { info: true, launch: true, controls: true };
        try {
    ...
    function initPanelVisibility() {
        ...
        document.addEventListener('click', (e) => {
            ...
            if (hideBtn) {
                ...
                if (k !== 'info' && k !== 'launch' && k !== 'controls') return;
                panelVis[k] = false;
                applyPanelVisibility();
                writePanelState();
                return;
    ...
    function toggleAllPanels() {
        const anyOn = panelVis.info || panelVis.launch || panelVis.controls;
        if (anyOn) {
            panelVis.info = false;
    ...
    function updateUI(metHours) {
        ...
        document.getElementById('warp-display').textContent =
    ...
    function onKeyDown(e) {
        if (e.key === 'Escape') clearFollowModes();
        if (e.key === 'h' || e.key === 'H') {
    ...
    window.setTimeWarp = function (w) {
        timeWarp = w;
        document.querySelectorAll('.warp-btn').forEach(b => b.classList.remove('active'));
        const btn = document.querySelector(`.warp-btn[data-warp="${w}"]`);
    ...
    window.jumpToNow = function () {
        ...
        clearFollowModes();
        document.querySelectorAll('.warp-btn').forEach(b => b.classList.remove('active'));
        const btn = document.querySelector('.warp-btn[data-warp="1"]');
        if (btn) btn.classList.add('active');
    };
